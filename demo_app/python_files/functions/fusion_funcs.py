...
def ff_pair(path_img1,path_img2,rest_models,rec_model):
    ff_df=pd.read_csv(r"demo_app\data\files\feature_fusion_scores.csv")
...
def sf_pair(path_img1,path_img2,rest_models,rec_model):
    sf_df = pd.read_csv(r"demo_app\data\files\score_fusion_file.csv")
    ...
    i = line.index[0] // 600
    score = line[col].iloc[0]
    threshold, acc, far, frr,_ = calcul_threshold_acc(sf_df[[col]], i)
    return score, threshold
...
def sf_fold(nb_fold,rest_models,rec_model):
    ...
    threshold, acc, far, frr,train = calcul_threshold_acc(sf_df[[col]], nb_fold)
    return acc, threshold,far,frr,train
...
def sf_all(rest_models,rec_model):
    ...
    total_acc = sum(ACC) / len(ACC)
    thr = sum(THR) / len(THR)
    return total_acc,thr,far,frr,sf_df[[col]]
...
def hf_pair(path_img1,path_img2,rest_models_lev1,rest_model_lev2,rec_model):
    hf_df = pd.read_csv(r"demo_app\data\files\hybrid_fusion_scores.csv")
    ...
    i = line.index[0] // 600
    score = line[col].iloc[0]
    threshold, acc, far, frr,_= calcul_threshold_acc(hf_df[[col]], i)
    return score, threshold
...
def hf_fold(nb_fold,rest_models_lev1,rest_model_lev2,rec_model):
    ...
    threshold, acc, far, frr,train= calcul_threshold_acc(hf_df[[col]], nb_fold)
    return acc, threshold,far,frr,train
# function to retrieve score fusion for al dataset
def hf_all(rest_models_lev1,rest_model_lev2,rec_model):
    ...
    thr = sum(THR) / len(THR)
    total_acc = sum(ACC) / len(ACC)
    return total_acc,thr,far,frr,hf_df[[col]]
